...
@cli.command()
@click.option('-w', '--workload', multiple=True, help="benchmark framework application datasize run_id, e.g., hpcc thor lr large 1")
...
@click.option('--ami', default='ami-a4fd9fdb', help="The default or customized AMI")
...
@click.option('--s3-bucket', default='scout-dataset-hpcc', help="The S3 bucket to store benchmark results")
...
mybenchmark()
{''' + '''
    /bin/bash {}/auto_hpcc.sh {}'''.format(script_dir, workload_str) + '''
}
